...
- **A plain `npm ci` works — keep it that way.** It used to fail with `ERESOLVE`:
  `cmdk`, `next-themes` and `vaul` all declared `react@^18` peers while the app runs
  React 19, and the lockfile pinned the exact versions that predated React 19 support.
  Those three were bumped and the rest of the tree re-resolved within its existing
  ranges. If you ever need `--legacy-peer-deps` again, a dependency has regressed into
  a React 19 peer conflict — fix the dependency rather than adding the flag back, or
  Vercel silently installs a tree npm considers invalid.
